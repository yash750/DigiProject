...
// Accessibility: htmlFor/id pairing, aria-describedby on errors, role="alert"
function Field({ label, name, value, onChange, type = "text", placeholder, error }) {
  return (
    <div className="form-group">
      ...
      />
      {error && (
        <span id={`${name}-err`} role="alert"
          style={{ fontSize: "0.75rem", color: "var(--danger)", marginTop: 4, display: "block" }}>
          ⚠ {error}
        </span>
      )}
    </div>
  ...
  return (
    <div style={{ display: "flex", gap: 28, flexWrap: "wrap", alignItems: "flex-start" }}>
      ...
      <div style={{ flex: "0 0 220px", display: "flex", flexDirection: "column", gap: 16 }}>
        {/* Aesthetic Integrity: avatar with initials, role badge, org card — visually balanced */}
        ...
        {org && (
          <div className="card" style={{ padding: "18px 20px" }}>
            <div style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 10 }}>
              Organization
            </div>
      ...
      <div style={{ flex: "1 1 360px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {/* Hierarchy: Edit Profile above Change Password — profile info is primary action */}
        <div className="form-card" style={{ maxWidth: "100%" }}>
          <div className="form-title">✏️ Edit Profile</div>
          <div className="form-subtitle">Update your personal information.</div>
          <form onSubmit={saveInfo} noValidate aria-label="Edit profile form">
        ...
        {/* Error Prevention: confirm field + current password required before change */}
        <div className="form-card" style={{ maxWidth: "100%" }}>
          <div className="form-title">🔒 Change Password</div>
          <div className="form-subtitle">Leave blank if you don't want to change your password.</div>
          <form onSubmit={savePassword} noValidate aria-label="Change password form">
            ...
            <button type="submit" className="btn btn-primary" disabled={savingPass} aria-busy={savingPass}>
